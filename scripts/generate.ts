...
const remixicon = path.normalize(path.dirname(require.resolve('remixicon/package.json'))).replaceAll('\\', '/')
...
const componentTemplate = (name: string, path: string) => `
...
const checkAllowedAttr = (attr: string, value: string) => {
...
const components = await Promise.all(
    files.map(async (file) => {
        ...
        const allPaths = [] as Record<string, string>[]
        while (true) {
